...
const heroImage = "https://images.unsplash.com/photo-1500530855697-b586d89ba3ee?q=80&w=2070&auto=format&fit=crop";
...
export default function HomePage() {
  ...
  const selectedPlan = useMemo(() => plans.find((plan) => plan.tier === tier && plan.billing === billing)!, [tier, billing]);
  ...
  async function subscribe() {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data.error || 'Checkout is not connected yet.');
  ...
  return (
    <main className="bg-black text-white min-h-screen overflow-hidden">
      ...
      <section className="relative min-h-screen flex items-center justify-center px-6" style={{ backgroundImage: `linear-gradient(to bottom, rgba(0,0,0,.35), rgba(0,0,0,.88)), url('${heroImage}')`, backgroundSize: 'cover', backgroundPosition: 'center' }}>
        <div className="absolute inset-0 bg-black/20" />
        <div className="relative max-w-6xl text-center pt-24">
          <p className="uppercase tracking-[0.45em] text-sm text-neutral-300 mb-6">Exclusive Fine Art Photography Club</p>
          <h1 className="text-7xl md:text-[10rem] font-extralight tracking-[0.2em] leading-none mb-8">BANGERS</h1>
          <p className="max-w-3xl mx-auto text-xl md:text-3xl text-neutral-200 leading-relaxed mb-12 font-light">
            Rare travel photography delivered quarterly.<br />Never posted online. Never sold publicly.<br />Only available to members.
          </p>
          <div className="flex flex-col sm:flex-row gap-5 justify-center">
            <a href="#membership" className="bg-white text-black px-10 py-5 rounded-2xl text-lg font-medium hover:scale-105 transition duration-300 shadow-2xl">Become a Collector</a>
            <a href="#concept" className="border border-white/30 px-10 py-5 rounded-2xl text-lg hover:bg-white/10 transition duration-300">What Is A Banger?</a>
          </div>
        </div>
      </section>
      ...
      <section id="concept" className="py-32 px-6 border-t border-white/10">
        <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-16 items-center">
          <div>
            ...
            <h2 className="text-5xl md:text-6xl font-extralight mb-10 leading-tight">You know it when you capture one.</h2>
          ...
          <div className="bg-neutral-950 border border-white/10 rounded-[2rem] p-8 shadow-[0_0_80px_rgba(255,255,255,0.05)]">
            <div className="border border-white/10 rounded-[1.5rem] p-8 bg-black">
              <div className="aspect-[4/5] rounded-2xl overflow-hidden mb-8">
                <img src="https://images.unsplash.com/photo-1506744038136-46273834b3fb?q=80&w=1974&auto=format&fit=crop" alt="Collector print example" className="w-full h-full object-cover hover:scale-105 transition duration-700" />
              </div>
              <h3 className="text-3xl font-extralight">First Light Over Moraine Lake</h3>
              <p className="text-neutral-400 text-lg mt-2">Banff National Park · September 2026</p>
              <p className="text-neutral-300 leading-relaxed text-lg font-light pt-4 mt-4 border-t border-white/10">Captured after hiking before sunrise through freezing rain. The clouds opened for less than two minutes before disappearing completely.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section id="membership" className="py-32 px-6 border-t border-white/10 bg-neutral-950">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-20">
            ...
            <p className="text-neutral-400 max-w-3xl mx-auto text-xl font-light leading-relaxed">No hard edition limits at launch — every member gets the quarterly BANGER release in their chosen size.</p>
          </div>
            ...
            {planGroups.map((group) => (
              <div key={group.tier} className="border border-white/10 rounded-[2rem] p-10 bg-black hover:-translate-y-2 hover:border-white/20 transition duration-300">
                ...
                <div className="space-y-5 mb-10">
                  <div className="flex justify-between border-b border-white/10 pb-4 text-lg"><span className="text-neutral-400">Quarterly</span><span>{group.quarterly}</span></div>
                  <div className="flex justify-between border-b border-white/10 pb-4 text-lg"><span className="text-neutral-400">Annual Discount</span><span>{group.annual}</span></div>
                  <div className="flex justify-between border-b border-white/10 pb-4 text-lg"><span className="text-neutral-400">US Shipping</span><span>$9.99</span></div>
                </div>
                <button onClick={() => { setTier(group.tier); document.getElementById('checkout')?.scrollIntoView({ behavior: 'smooth' }); }} className="w-full bg-white text-black py-5 rounded-2xl text-lg font-medium hover:scale-[1.02] transition duration-300 shadow-xl">Select {group.name}</button>
              </div>
            ))}
          </div>
          ...
          <div id="checkout" className="bg-black border border-white/10 rounded-[2rem] p-10 md:p-14 shadow-[0_0_80px_rgba(255,255,255,0.05)]">
            <div className="grid md:grid-cols-2 gap-12">
              <div>
                ...
                <p className="text-neutral-300 text-lg leading-relaxed font-light">Choose your edition and billing cycle. Stripe securely handles payment, tax, US shipping address, receipts, and recurring billing.</p>
              </div>
              <div className="bg-neutral-950 border border-white/10 rounded-[2rem] p-8 space-y-6">
                ...
                <input type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="collector@email.com" className="w-full bg-black border border-white/10 rounded-2xl px-5 py-4 text-white placeholder:text-neutral-600" />
                <div className="pt-4 border-t border-white/10">
                  <div className="flex justify-between text-lg mb-2"><span className="text-neutral-400">Membership</span><span>{selectedPlan.amount} / {selectedPlan.billing === 'quarterly' ? 'quarter' : 'year'}</span></div>
                  <div className="flex justify-between text-sm text-neutral-500 mb-6"><span>{selectedPlan.name}</span><span>{selectedPlan.size}</span></div>
                  <button onClick={subscribe} disabled={loading} className="w-full bg-white text-black py-5 rounded-2xl text-xl font-medium hover:scale-[1.02] transition duration-300 shadow-xl disabled:opacity-50">{loading ? 'Opening Stripe...' : 'Continue to Stripe Checkout'}</button>
                  <p className="text-center text-neutral-500 text-sm mt-5 leading-relaxed">US shipping is $9.99. Checkout activates after Stripe keys are added.</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <section id="experience" className="py-32 px-6 border-t border-white/10">
        <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-20 items-center">
          <div>
            ...
            <h2 className="text-5xl md:text-7xl font-extralight leading-tight mb-8">More than a print.</h2>
            <p className="text-xl text-neutral-300 leading-relaxed font-light">Every shipment is designed to feel intentional, cinematic, and personal. Prints are produced in-house using archival fine art materials, carefully packaged, and shipped directly from the studio.</p>
          </div>
          <div className="relative aspect-[4/5] overflow-hidden rounded-[2rem] border border-white/10">
            <img src="https://images.unsplash.com/photo-1510798831971-661eb04b3739?q=80&w=1974&auto=format&fit=crop" alt="Premium print packaging" className="w-full h-full object-cover hover:scale-105 transition duration-700" />
          </div>
        </div>
      </section>
      ...
      <footer className="border-t border-white/10 py-12 px-6">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-6">
          <div><h3 className="text-2xl tracking-[0.35em] font-light mb-2">BANGERS</h3><p className="text-neutral-500">Exclusive fine art travel photography.</p></div>
          <div className="flex gap-8 text-neutral-400 uppercase tracking-[0.2em] text-sm"><a href="#membership">Membership</a><a href="#faq">FAQ</a><a href="mailto:hello@bangersprints.com">Contact</a></div>
        </div>
      </footer>
    </main>
  );
